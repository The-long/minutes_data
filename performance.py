...
def factor_weights_min(factor_data,
                   demeaned=True,
                   group_adjust=False,
                   equal_weight=False):
    """
...
def factor_returns_min(factor_data,
                   demeaned=True,
                   group_adjust=False,
                   equal_weight=False,
                   by_asset=False):
    ...
    weights = \
        factor_weights_min(factor_data, demeaned, group_adjust, equal_weight)
    ...
    if by_asset:
        ...
        returns = weighted_returns.groupby(level=['date','min']).sum()
...
def factor_alpha_beta_min(factor_data,
                      returns=None,
                      demeaned=True,
                      group_adjust=False,
                      equal_weight=False):
    ...
    universe_ret = factor_data.groupby(['date','min'])[
        utils.get_forward_returns_columns(factor_data.columns)] \
        .mean().loc[returns.index]
...
def cumulative_returns_min(returns):
...
def mean_return_by_quantile_min(factor_data,
                            by_date=False,
                            by_group=False,
                            demeaned=True,
                            group_adjust=False):
    ...
    if group_adjust:
        grouper = [factor_data.index.get_level_values('date')] + ['group']
        factor_data = utils.demean_forward_returns_min(factor_data, grouper)
    elif demeaned:
        factor_data = utils.demean_forward_returns_min(factor_data)
    else:
        factor_data = factor_data.copy()

    grouper = ['factor_quantile', 'date', 'min']
...
def create_pyfolio_input_min(factor_data,
                         period,
                         capital=None,
                         long_short=True,
                         group_neutral=False,
