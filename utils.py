...
def quantize_factor_min(factor_data,
                    quantiles=5,
                    bins=None,
                    by_group=False,
                    no_raise=False,
    ...
    grouper = [factor_data.index.get_level_values('min')]
    if by_group:
        grouper.append('group')
...
def demean_forward_returns_min(factor_data, grouper=None):
    """
    Convert forward returns to returns relative to mean
    period wise all-universe or group returns.
    group-wise normalization incorporates the assumption of a
...
def get_clean_factor_min(factor,
                     forward_returns,
                     groupby=None,
                     binning_by_group=False,
                     quantiles=5,
    ...
    factor_copy = factor.copy()
    factor_copy.index = factor_copy.index.rename(['date', 'min','asset'])
    ...
    no_raise = False if max_loss == 0 else True
    quantile_data = quantize_factor_min(
        merged_data,
        quantiles,
        bins,
        binning_by_group,
...
def get_clean_factor_and_forward_returns_min(factor,
                                         prices,
                                         groupby=None,
                                         binning_by_group=False,
                                         quantiles=5,
    ...
    forward_returns = compute_forward_returns_min(factor, prices, periods,
                                              filter_zscore,
                                              cumulative_returns)

    factor_data = get_clean_factor_min(factor, forward_returns, groupby=groupby,
                                   groupby_labels=groupby_labels,
                                   quantiles=quantiles, bins=bins,
                                   binning_by_group=binning_by_group,
                                   max_loss=max_loss, zero_aware=zero_aware)
