...
def create_returns_tear_sheet(factor_data,
    ...
    return mean_quant_ret, factor_returns 

def create_returns_tear_sheet_min(factor_data,
                              long_short=True,
                              group_neutral=False,
                              by_group=False):
    """
    ...
    factor_returns = perf.factor_returns_min(factor_data,
                                         long_short,
                                         group_neutral)

    mean_quant_ret, std_quantile = \
        perf.mean_return_by_quantile_min(factor_data,
                                     by_group=False,
                                     demeaned=long_short,
                                     group_adjust=group_neutral)
    ...
    alpha_beta = perf.factor_alpha_beta_min(factor_data,
                                        factor_returns,
                                        long_short,
                                        group_neutral)
    ...
    for p in factor_returns:
        ...
        pm.plot_cumulative_returns(factor_returns[p], title=title, ax=gf.next_row())

        pm.plot_cumulative_returns_by_quantile(
            mean_quant_ret_bydate[p],
            title=title,
            ax=gf.next_row()
        )
